...
import { commonSignatureAxes } from "../../charts/evidence-adapters"
...
const axes = computed(() => commonSignatureAxes(games.value))
...
const signatures = computed(() => games.value.map((game) => {
  ...
  return {
    value: axes.value.map((axis) => Math.round(
      game.components.find((component) => component.key === axis.key)!.percentile * 100,
    )),
    game,
    history,
    lineStyle: {
