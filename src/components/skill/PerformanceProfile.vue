...
const selectedMetrics = computed(() =>
  [...(selected.value?.metrics ?? [])].sort((left, right) => right.weight - left.weight),
)

const confidenceLabel = (confidence: PerformanceConfidence) => ({
  learning: "Learning",
  ...
  <section class="dimensions card" aria-labelledby="rvi-title">
    <header class="dimensions-head">
      <div>
        ...
        <p class="intro">
          RVI turns your match data into a 0–100 score for fighting, survival, resources,
          map control, initiative, consistency, and versatility. Small samples stay near 50.
        </p>
      </div>
    ...
    <div class="dimension-grid" aria-label="RVI performance vectors">
      ...
      >
        ...
        <span class="dimension-copy">
          ...
          <small>{{ confidenceLabel(dimension.confidence) }} · {{ dimension.games }} games</small>
        </span>
    ...
    <section v-if="selected" class="dimension-detail">
      ...
      <div v-if="detailsOpen" :id="`dimension-${selected.key}`" class="detail-body">
        ...
        <div class="measurement-table">
          ...
          <article v-for="metric in selectedMetrics" :key="metric.key" class="measurement-row">
            <div class="measurement-copy">
              <strong>{{ metric.label }}</strong>
              <p>{{ metric.description }}</p>
              <small>{{ metric.comparison }} · {{ metric.games }} {{ metric.games === 1 ? "game" : "games" }}</small>
            </div>
            <div class="measurement-score">
              <div
                class="segment-meter"
                role="img"
                :aria-label="`${metric.score} out of 100`"
              >
                <i
                  v-for="segment in 10"
                  :key="segment"
                  :class="{ filled: metric.score >= segment * 10 - 5 }"
                  aria-hidden="true"
                />
              </div>
            ...
            <div class="measurement-influence">
              <strong class="numeric">{{ Math.round(metric.weight * 100) }}%</strong>
              <small>of {{ selected.label }}</small>
            </div>
          </article>
        </div>
        <p class="method-note">
          RVI uses every available measurement shown here. Missing evidence is omitted and the remaining weights rebalance; missing never means zero.
        </p>
      </div>
    </section>
  </section>
