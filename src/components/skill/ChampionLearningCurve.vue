...
const moving = computed(() => games.value.map((game, index) => {
  ...
  return [index + 1, window.reduce((sum, entry) => sum + entry.recallScore!, 0) / window.length, game.win]
}))
...
<template>
  <div class="learning-curve">
    ...
    <BaseEChart
      v-if="games.length"
      :option="option"
      ariaLabel="Champion learning curve showing each graded match and a five-game moving Recall Score on a zero-to-one-hundred scale."
      height="310px"
    />
