...
const option = computed<EChartsCoreOption>(() => ({
  ...
  tooltip: {
    ...
    formatter: (raw: unknown) => {
      ...
      if (item.seriesName === "5-minute average") return `<strong>${item.value[0]} minutes</strong><br/>Average Recall Score ${item.value[1].toFixed(1)}`
      const game = graded.value[item.dataIndex]
      return game ? `<strong>${escapeTooltip(championNameById(null, game.championId))}</strong><br/>${(game.durationSecs / 60).toFixed(1)} minutes · ${escapeTooltip(game.grade ?? "–")} · Recall Score ${game.recallScore?.toFixed(1)}<br/>${game.win ? "Win" : "Loss"}` : ""
    },
  },
...
<template>
  <BaseEChart
    :option="option"
    ariaLabel="Recall Score by match duration on a zero-to-one-hundred scale. Green points are wins, red points are losses, and the line averages five-minute bands."
    height="310px"
  />
</template>
