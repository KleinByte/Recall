...
import { SUMMARY_METRIC_KEYS_V3, type SummaryMetricKeyV3 } from "./rvi-v3-summary.js"
...
export const TIMELINE_METRIC_KEYS_V3 = [
...
export type TimelineMetricKeyV3 = typeof TIMELINE_METRIC_KEYS_V3[number]
export type MetricKeyV3 = SummaryMetricKeyV3 | TimelineMetricKeyV3

export interface MetricContextV3 {
  context: GradeModeContextV3
  position?: Position | "UNKNOWN"
  archetype?: PrimaryArchetype
}

export interface MetricDefinitionV3<TMetricKey extends string = MetricKeyV3> {
  key: TMetricKey
  label: string
  description: string
  formula: string
  ...
  applicable(context: MetricContextV3): boolean
}

export interface RviMetricPolicyV3<TMetricKey extends string = MetricKeyV3> {
...
const isRift = (context: MetricContextV3) => context.context.ruleset !== "howling_abyss"
const hasRiftPosition = (context: MetricContextV3) =>
  isRift(context) && context.position !== undefined && context.position !== "UNKNOWN"
const hasLane = (context: MetricContextV3) =>
...
const applicableFor = (policy: DefinitionSeed["applicability"]) => {
  ...
  if (policy === "non_utility") return (context: MetricContextV3) =>
    context.position !== "UTILITY"
  return () => true
}

const definition = (seed: DefinitionSeed): MetricDefinitionV3 => {
  const { applicability, ...values } = seed
  return Object.freeze({ ...values, applicable: applicableFor(applicability) })
}

const DEFINITIONS: readonly MetricDefinitionV3[] = Object.freeze([
  ...
  ...([10, 15, 20, 30] as const).flatMap((minute) => [
    definition({ key: `gold_delta_${minute}` as MetricKeyV3, label: `Gold delta at ${minute}`, description: `Gold difference from the exact opposing role near ${minute}:00.`, formula: `owner gold - opposing-role gold at ${minute}:00 (nearest frame within 30 seconds)`, unit: "gold", direction: "higher", source: "timeline", applicability: "rift_position" }),
    definition({ key: `cs_delta_${minute}` as MetricKeyV3, label: `CS delta at ${minute}`, description: `CS difference from the exact opposing role near ${minute}:00.`, formula: `owner CS - opposing-role CS at ${minute}:00 (nearest frame within 30 seconds)`, unit: "cs", direction: "higher", source: "timeline", applicability: "rift_position" }),
  ]),
...
export const METRIC_DEFINITIONS_V3 = DEFINITIONS
export const RVI_V3_METRIC_POLICIES = POLICIES

export function metricDefinitionV3(metricKey: string): MetricDefinitionV3 | undefined {
  return DEFINITION_BY_KEY.get(metricKey as MetricKeyV3)
}

export function rviMetricPolicyV3(metricKey: string): RviMetricPolicyV3 | undefined {
  return POLICY_BY_KEY.get(metricKey as MetricKeyV3)
}

export function assertValidMetricRegistryV3(
  definitions: readonly MetricDefinitionV3[] = DEFINITIONS,
  policies: readonly RviMetricPolicyV3[] = POLICIES,
): void {
  ...
  const vectors = new Set<string>(RVI_CAPABILITY_VECTORS_V3)
  ...
  for (const [key, direction] of Object.entries(GRADE_V3_METRIC_DIRECTIONS)) {
