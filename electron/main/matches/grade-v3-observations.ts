...
import {
  ...
  type DetailMetricCalibrationRowV3,
...
export interface GradeCalibrationClusterFactsV3 {
  gameId: number
  puuid: string
 ...
 */
export function gradeCalibrationClusterIdV3(
  facts: GradeCalibrationClusterFactsV3,
): string {
...
export interface GradeRawParticipantV3 {
...
export interface GradeRawLobbyV3 {
  ...
  players: readonly GradeRawParticipantV3[]
  /** Optional expanded RVI evidence derived from the same retained match. */
  detailMetricObservations?: ReadonlyMap<
    number,
    readonly RawMetricObservationV3[]
  >
}

export interface GradeMetricObservationV3 {
  clusterId: string
  trackedMode: string
  scopeKey: string
  position: NormalizedPosition
...
export interface GradeCompositeObservationV3 {
  clusterId: string
  trackedMode: string
  scopeKey: string
  position: NormalizedPosition
...
export interface GradeCalibrationSnapshotV3 {
  formatVersion: typeof GRADE_V3_CALIBRATION_FORMAT_VERSION
  referencePopulation: {
    kind: "local_recall_installation"
    clusterUnit: "match"
    clusterIdentity: typeof GRADE_V3_RECIPE.calibration.clusterIdentity
    frozen: true
  ...
  clusterIds: string[]
  observations: Record<GradeMetricV3, GradeMetricObservationV3[]>
  /** Frozen raw distributions for inspectable RVI metrics beyond Grade's core. */
  detailObservations: Record<string, GradeMetricObservationV3[]>
  compositeObservations: GradeCompositeObservationV3[]
}

export interface PreparedGradeParticipantV3 {
  participantId: number
  teamId: number
  isPlayer: boolean
  championId: number
...
export interface PreparedGradeLobbyV3 {
  players: PreparedGradeParticipantV3[]
...
export interface GradeCalibrationScopeSummaryV3 {
  scopeKey: string
  trackedMode: string
  rulesetKey: string
 ...
 */
export function deriveRawMetricEvidenceV3(
  lobby: GradeRawLobbyV3,
): Map<number, Partial<Record<GradeMetricV3, Evidence<number>>>> {
  const result = new Map<number, Partial<Record<GradeMetricV3, Evidence<number>>>>()
  ...
  if (lobby.detailMetricObservations) {
    ...
    for (const player of lobby.players) {
      ...
      const evidence: Partial<Record<GradeMetricV3, Evidence<number>>> = {}
...
function emptyObservationRecord(): Record<GradeMetricV3, GradeMetricObservationV3[]> {
  const observations = {} as Record<GradeMetricV3, GradeMetricObservationV3[]>
  for (const metric of GRADE_METRICS) observations[metric] = []
  return observations
}

/** Counts independent matches in the exact mode/rules epoch used for calibration. */
export function summarizeGradeCalibrationScopesV3(
  lobbies: readonly GradeRawLobbyV3[],
): GradeCalibrationScopeSummaryV3[] {
  ...
  return [...scopes.entries()]
    .map(([scopeKey, entry]) => ({
      ...
      supported: entry.clusterIds.size >= GRADE_V3_MINIMUM_SCOPE_MATCHES,
    }))
...
export function buildGradeCalibrationSnapshotV3(
  lobbies: readonly GradeRawLobbyV3[],
): GradeCalibrationSnapshotV3 {
  const observations = emptyObservationRecord()
  const detailObservations: Record<string, GradeMetricObservationV3[]> = {}
  const scopeSummaries = summarizeGradeCalibrationScopesV3(lobbies)
  ...
  for (const lobby of sorted) {
    const scopeKey = calibrationScopeKey(lobby.context)
    const evidence = deriveRawMetricEvidenceV3(lobby)
  ...
  const snapshot: GradeCalibrationSnapshotV3 = {
    formatVersion: GRADE_V3_CALIBRATION_FORMAT_VERSION,
    referencePopulation: {
      kind: "local_recall_installation",
      clusterUnit: "match",
      clusterIdentity: GRADE_V3_RECIPE.calibration.clusterIdentity,
      frozen: true,
...
const metricDirection = (metric: GradeMetricV3) => GRADE_V3_METRIC_DIRECTIONS[metric]

const toCalibrationObservations = (
  entries: readonly GradeMetricObservationV3[],
): CalibrationObservation[] => entries.map((entry) => ({
  matchId: entry.clusterId,
  value: entry.value,
}))

interface CohortRowIndexV3<T extends GradeMetricObservationV3> {
  scopes: Map<string, T[]>
...
function indexCohortRows<T extends GradeMetricObservationV3>(
  rows: readonly T[],
): CohortRowIndexV3<T> {
  const index: CohortRowIndexV3<T> = {
    scopes: new Map(),
    positions: new Map(),
    archetypes: new Map(),
...
const metricIndexes = new WeakMap<
  GradeCalibrationSnapshotV3,
  Record<GradeMetricV3, CohortRowIndexV3<GradeMetricObservationV3>>
>()
const compositeIndexes = new WeakMap<
  GradeCalibrationSnapshotV3,
  CohortRowIndexV3<GradeCompositeObservationV3>
>()
const detailCalibrationRows = new WeakMap<
  GradeCalibrationSnapshotV3,
  Map<string, DetailMetricCalibrationRowV3[]>
>()

function metricIndexFor(snapshot: GradeCalibrationSnapshotV3) {
  const cached = metricIndexes.get(snapshot)
  if (cached) return cached
  const index = {} as Record<
    GradeMetricV3,
    CohortRowIndexV3<GradeMetricObservationV3>
  >
  for (const metric of GRADE_METRICS) {
...
function compositeIndexFor(snapshot: GradeCalibrationSnapshotV3) {
...
export interface PreparedDetailMetricObservationV3 extends RawMetricObservationV3 {
 ...
 */
export function prepareDetailMetricObservationsFromSnapshotV3(
  lobby: GradeRawLobbyV3,
  snapshot: GradeCalibrationSnapshotV3,
): ReadonlyMap<number, readonly PreparedDetailMetricObservationV3[]> {
  const result = new Map<number, PreparedDetailMetricObservationV3[]>()
  const scopeKey = calibrationScopeKey(lobby.context)
  for (const player of lobby.players) {
    const archetype = resolvePrimaryArchetypeWithSource(
      player.championId,
    ...
    const rows: PreparedDetailMetricObservationV3[] = []
    for (const raw of lobby.detailMetricObservations?.get(player.participantId) ?? []) {
      const definition = metricDefinitionV3(raw.metricKey)
      if (!definition) {
        rows.push({
          ...raw,
...
function prepareMetricLobbyFromSnapshotV3(
  lobby: GradeRawLobbyV3,
  snapshot: GradeCalibrationSnapshotV3,
): PreparedGradeLobbyV3 {
  const raw = deriveRawMetricEvidenceV3(lobby)
  const players: PreparedGradeParticipantV3[] = []
  const scopeKey = calibrationScopeKey(lobby.context)
  let observedCount = 0
  ...
  for (const player of lobby.players) {
    ...
    const metricEvidence: Partial<Record<GradeMetricV3, Evidence<number>>> = {}
    const metricProvenance: PreparedGradeParticipantV3["metricProvenance"] = {}
    for (const metric of GRADE_METRICS) {
      ...
      if (referenceMatches < GRADE_V3_MINIMUM_REFERENCE_MATCHES) {
 ...
 * raw metric -> metric percentile -> responsibility composite -> RoleFit ECDF.
 */
export function prepareGradeLobbyFromSnapshotV3(
  lobby: GradeRawLobbyV3,
  snapshot: GradeCalibrationSnapshotV3,
): PreparedGradeLobbyV3 {
  const prepared = prepareMetricLobbyFromSnapshotV3(lobby, snapshot)
  ...
  for (const player of prepared.players) {
    ...
    const calibrated = shrunkMidEcdf(rawScore.rawResponsibilityComposite, cohort, {
      excludeMatchId: lobby.clusterId,
      rootKappa: GRADE_V3_RECIPE.calibration.finalCompositeRootKappa,
    })
