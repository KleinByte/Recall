...
export const RVI_V3_SUMMARY_DERIVATION_ID =
  "recall.rvi.v3.summary.2026-08-09.r1" as const

export const SUMMARY_METRIC_KEYS_V3 = [
  "damage_share",
  "kill_participation",
  "deaths_per_10",
  "gold_per_min",
...
export type SummaryMetricKeyV3 = typeof SUMMARY_METRIC_KEYS_V3[number]

export interface SummaryMetricParticipantV3 {
...
export interface SummaryMetricDerivationInputV3 {
  participantId: number
  durationSecs: number
  context: GradeModeContextV3
  /** A complete lobby is required for team-share observations. */
  participants: readonly SummaryMetricParticipantV3[]
  sourceQuality?: MetricSourceQualityV3
}

export type SummaryMetricObservationV3 = RawMetricObservationV3<SummaryMetricKeyV3>

const finiteNonnegative = (value: number | undefined): value is number =>
  typeof value === "number" && Number.isFinite(value) && value >= 0

const riftObjectiveEvidence = (context: GradeModeContextV3) =>
...
function allUnavailable(
  ...
  return SUMMARY_METRIC_KEYS_V3.map((metricKey) => raw(
    metricKey,
    "unknown",
 ...
 */
export function deriveSummaryMetricObservationsV3(
  input: SummaryMetricDerivationInputV3,
): SummaryMetricObservationV3[] {
  ...
  const output: SummaryMetricObservationV3[] = []
  const add = (
    key: SummaryMetricKeyV3,
    unit: string,
    evidence: Evidence<number>,
    source: MetricSourceV3 = "scoreboard",
    details: Pick<RawMetricObservationV3, "numerator" | "denominator" | "opportunityCount"> = {},
  ...
  const teamShare = (
    key: SummaryMetricKeyV3,
    value: number | undefined,
    select: (entry: SummaryMetricParticipantV3) => number | undefined,
    zeroReason: string,
    source: MetricSourceV3 = "scoreboard",
  ) => {
...
export function summaryMetricEvidenceByKeyV3(
  input: SummaryMetricDerivationInputV3,
): ReadonlyMap<SummaryMetricKeyV3, SummaryMetricObservationV3> {
  return new Map(deriveSummaryMetricObservationsV3(input).map((entry) => [
    entry.metricKey,
    entry,
  ]))
}
