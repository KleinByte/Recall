...
export const RVI_OBJECTIVE_FRAME_TOLERANCE_MS = 60_000
export const RVI_OBJECTIVE_PROXIMITY_RADIUS = 1_500
export const RVI_PRE_OBJECTIVE_WINDOW_MS = 60_000
export const RVI_PRE_OBJECTIVE_RADIUS = 2_500
export const RVI_EARLY_END_MS = 15 * 60_000

export interface TimelineMetricDerivationInput {
...
export type TimelineMetricObservation = RawMetricObservation<TimelineMetricKey>

export interface TimelineFightCluster {
...
const isRift = (context: MatchGradeModeContext) => context.ruleset !== "howling_abyss"
...
function raw(
  ...
  return {
    metricKey,
    rawEvidence,
    unit: metricDefinition(metricKey)?.unit ?? "unknown",
    source: "timeline",
    sourceQuality,
    ...details,
  }
...
function teamEntries(
  input: TimelineMetricDerivationInput,
): Map<number, number> {
...
export function clusterTimelineFights(
  events: readonly CompactTimelineEvent[],
): Evidence<readonly TimelineFightCluster[]> {
  ...
  for (let left = 0; left < ordered.length; left += 1) {
    for (let right = left + 1; right < ordered.length; right += 1) {
      if (ordered[right].timestamp - ordered[left].timestamp > RVI_FIGHT_CLUSTER_WINDOW_MS) break
      const a = ordered[left].position as Point
      const b = ordered[right].position as Point
      if (Math.hypot(a.x - b.x, a.y - b.y) <= RVI_FIGHT_CLUSTER_RADIUS) {
  ...
  const clusters = [...components.values()].map((cluster): TimelineFightCluster => {
...
function participantPointNear(
  ...
  participantId: number,
  timestamp: number,
  toleranceMs = RVI_OBJECTIVE_FRAME_TOLERANCE_MS,
): Point | undefined {
...
export function riftLaneAt(point: Point): LanePosition | undefined {
  const { x, y } = point
...
function allUnavailable(
  ...
  return TIMELINE_METRIC_KEYS.map((key) => raw(key, timelineUnavailable(reason), sourceQuality))
}

/** Derives every corrected timeline proxy without applying a display ceiling. */
export function deriveTimelineMetricObservations(
  input: TimelineMetricDerivationInput,
): TimelineMetricObservation[] {
  const quality = input.sourceQuality ?? "retained"
  const timeline = input.timeline
  if (!timeline) {
    return TIMELINE_METRIC_KEYS.map((key) => {
      const applicable = metricDefinition(key)?.applicable({
  ...
  const values = new Map<TimelineMetricKey, TimelineMetricObservation>()
  const set = (
    key: TimelineMetricKey,
    evidence: Evidence<number>,
    details: Pick<RawMetricObservation, "numerator" | "denominator" | "opportunityCount"> = {},
  ) => values.set(key, raw(key, evidence, quality, details))
  const setRate = (key: TimelineMetricKey, numerator: number, denominator: number, reason: string) =>
  ...
  const clustersEvidence = incompleteSupplementalKills > 0
    ? unavailable<readonly TimelineFightCluster[]>(
      `incomplete_supplemental_kill_coverage:${incompleteSupplementalKills}`,
      { source: "legacy" },
    )
    : clusterTimelineFights(retainedKills)
  ...
  if (clustersEvidence.state !== "observed") {
    ...
    if (incompleteTeams) {
      ...
      const outcomeRate = (
        key: TimelineMetricKey,
        selected: readonly TimelineFightCluster[],
        reason: string,
      ) => {
  ...
  const phaseMetric = (
    ...
    const frame = nearestFrame(timeline.frames, timestamp, RVI_PHASE_FRAME_TOLERANCE_MS)
  ...
  const participationRate = (
    ...
    for (const event of selected) {
      ...
      if (Math.hypot(ownerPoint.x - event.position.x, ownerPoint.y - event.position.y) <=
          RVI_OBJECTIVE_PROXIMITY_RADIUS) count += 1
    }
  ...
  if (!isRift(input.context)) {
      ...
      const proximities = teamObjectives.map((event) => {
        ...
        return ownerPoint && finitePoint(event.position)
          ? Math.hypot(ownerPoint.x - event.position.x, ownerPoint.y - event.position.y) <=
            RVI_OBJECTIVE_PROXIMITY_RADIUS
          : undefined
      })
    ...
    if (objectiveTeamUnknown) {
      ...
      const deltas = teamBarons.map((event) => {
        const start = nearestFrame(timeline.frames, event.timestamp,
          RVI_OBJECTIVE_FRAME_TOLERANCE_MS)
    ...
    if (teamObjectives.length === 0) {
      ...
      const prepared = teamObjectives.filter((objective) => wards.some((ward) => {
        ...
        return Math.hypot(objective.position.x - ward.position.x,
          objective.position.y - ward.position.y) <= RVI_OBJECTIVE_PROXIMITY_RADIUS
      })).length
    ...
    const earlyTeamStructures = teamStructures.filter((event) => event.timestamp < RVI_EARLY_END_MS)
    if (structureTeamUnknown) {
    ...
    participationRate("early_objective_participation",
      teamObjectives.filter((event) => event.timestamp < RVI_EARLY_END_MS),
      "team_secured_no_early_objectives")
  }

  const earlyTeamKills = kills.filter((event) =>
    event.timestamp < RVI_EARLY_END_MS && eventTeam(event, teams) === input.teamId)
  const earlyTeamIdentityMissing = kills.some((event) =>
    event.timestamp < RVI_EARLY_END_MS && eventTeam(event, teams) === undefined)
  if (!isRift(input.context)) {
  ...
  const earlyContributions = contributions.filter((event) => event.timestamp < RVI_EARLY_END_MS)
  if (!isRift(input.context) || (input.position !== "TOP" && input.position !== "MIDDLE" &&
    ...
    const qualifying = earlyContributions.filter((event) => {
      ...
      return riftLaneAt(ownerPoint) !== input.position &&
  ...
  return TIMELINE_METRIC_KEYS.map((key) => values.get(key) ??
    raw(key, timelineUnavailable("timeline_metric_derivation_incomplete"), quality))
}

export function timelineMetricEvidenceByKey(
  input: TimelineMetricDerivationInput,
): ReadonlyMap<TimelineMetricKey, TimelineMetricObservation> {
  return new Map(deriveTimelineMetricObservations(input).map((entry) => [
    entry.metricKey,
    entry,
  ]))
}
