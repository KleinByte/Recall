...
import {
  GRADE_FAMILIES,
  GRADE_V3_TAXONOMY_VERSION,
  type GradeFamilyV3,
  type ResponsibilityTier,
} from "./grade-v3-recipe.js"

export { GRADE_V3_TAXONOMY_VERSION }
...
export type GradeRulesetV3 = "standard_sr" | "howling_abyss" | "league_classic"

export interface GradeModeContextV3 {
  modeFamily: ModeFamily
  /** Tracked mode separates ranked, normal, swiftplay, and future modes. */
  trackedMode: string
  ruleset: GradeRulesetV3
  /** Versioned rules epoch; intentionally coarser than a fine patch cell. */
  rulesetKey: string
}

const RULESET_BY_FAMILY: Readonly<Record<Exclude<ModeFamily, "other">, GradeRulesetV3>> =
  Object.freeze({
    sr: "standard_sr",
    aram: "howling_abyss",
...
export function isSupportedModeContext(context: GradeModeContextV3): boolean {
...
export function calibrationScopeKey(context: GradeModeContextV3): string {
...
export interface ArchetypeResolution {
  ...
  taxonomyVersion: typeof GRADE_V3_TAXONOMY_VERSION
}
...
export function resolvePrimaryArchetypeWithSource(
  ...
  if (explicit) {
    return { archetype: explicit, source: "explicit", taxonomyVersion: GRADE_V3_TAXONOMY_VERSION }
  }
  ...
  if (curated) {
    return { archetype: curated, source: "curated", taxonomyVersion: GRADE_V3_TAXONOMY_VERSION }
  }
...
export function responsibilityTiersFor(
  ...
  const combined = Object.fromEntries(GRADE_FAMILIES.map((family) => [
    family,
    positionProfile
      ? maxTier(archetypeProfile[family], positionProfile[family])
      : archetypeProfile[family],
  ])) as Record<GradeFamilyV3, ResponsibilityTier>
