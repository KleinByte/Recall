...
Adapted from bsowlx/DeepestLeague at revision
8cb084f6ae9a89362d30dc2200e775d91cf66f64 (MIT). The adaptation uses
Recall's local Data Dragon roster, deterministic seeds, and focused sampling.
...
def _initialize_worker(generator_instance):
    """Install immutable generator assets once instead of serializing them per task."""
    global _WORKER_GENERATOR
    _WORKER_GENERATOR = generator_instance
    cv2.setNumThreads(0)


def _worker_entry_point(args):
    """Worker entrypoint used by multiprocessing pool."""
    split_name, index, allow_overlap, base_seed = args
    generator_instance = _WORKER_GENERATOR
    if generator_instance is None:
        raise RuntimeError("Synthetic generator worker was not initialized")
    split_index = generator_instance.split_names.index(split_name)
    seed = int(np.random.SeedSequence([base_seed, split_index, index]).generate_state(1)[0])
    np.random.seed(seed)
    random.seed(seed)
    BACKGROUND_AUGMENT.set_random_seed(seed)
    ICON_AUGMENT.set_random_seed(seed + 1)
...
class SyntheticDataGenerator:
    ...
    def __init__(
        self,
        focus_champions: list[str] | None = None,
        focus_per_image: int = 1,
        seed: int = 42,
    ):
        self.resource_dir = str(TRAINING_ROOT / "assets")
        self.champion_dir = str(REPOSITORY_ROOT / "resources" / "champion-portraits")
        self.roster_path = TRAINING_ROOT / "roster.json"
        self.split_names = ["train", "val", "test"]

        roster = json.loads(self.roster_path.read_text(encoding="utf-8"))
        self.champion_names = [str(entry["assetKey"]) for entry in roster["classes"]]
        if len(self.champion_names) != int(roster["classCount"]):
            raise RuntimeError("Training roster class count is inconsistent")
        requested_focus = roster["focusChampions"] if focus_champions is None else focus_champions
        champion_lookup = {name.lower(): name for name in self.champion_names}
        unknown_focus = [name for name in requested_focus if name.lower() not in champion_lookup]
        if unknown_focus:
            raise ValueError(f"Focus champions are absent from the roster: {', '.join(unknown_focus)}")
        self.focus_champions = [champion_lookup[name.lower()] for name in requested_focus]
        self.focus_per_image = max(0, min(10, int(focus_per_image)))
        self.seed = int(seed)
        ...
        self.focus_indices = [self.champion_names.index(name) for name in self.focus_champions]
    ...
    def generate_data(
        ...
        for split_size, split_name in zip([self.n_train, self.n_val, self.n_test], self.split_names):
            for index in range(split_size):
                tasks.append((split_name, index, allow_icon_overlap, self.seed))

        total_tasks = len(tasks)
        print(f"Starting generation of {total_tasks} images using {workers} cores...")

        chunk_size = max(1, total_tasks // (workers * 4)) if total_tasks else 1

        if workers == 1:
            _initialize_worker(self)
            results = map(_worker_entry_point, tasks)
            ...
            with multiprocessing.Pool(
                ...
                for i, _ in enumerate(
                    pool.imap_unordered(_worker_entry_point, tasks, chunksize=chunk_size),
                    start=1,
                ):
                    if i % 100 == 0 or i == total_tasks:
                        print(f"\rProgress: {i}/{total_tasks} ({(i/total_tasks)*100:.1f}%)", end="")
        ...
        self._write_generation_manifest(allow_icon_overlap)
    ...
    def _generate_single_sample(self, split_name, index, allow_icon_overlap):
        ...
        chosen_champ_indices = self._sample_champion_indices(split_name, index)
        ...
        if np.random.uniform() < 0.05:
            ...
            required_slots = {
                slot for slot, champion_index in enumerate(chosen_champ_indices)
                if int(champion_index) in self.focus_indices
            }
            active_indices = sorted(set(active_indices) | required_slots)
    ...
    def _load_resources(self):
        """
        Loads:
          - champion icons from resources/champion-portraits/*.png
        ...
        for champion_name in self.champion_names:
            icon_path = os.path.join(self.champion_dir, f"{champion_name}.png")
            icon = cv2.imread(icon_path, cv2.IMREAD_UNCHANGED)
            if icon is None:
                raise FileNotFoundError(f"Champion portrait is missing or unreadable: {icon_path}")
            if icon.shape[0] <= boundary_crop * 2 or icon.shape[1] <= boundary_crop * 2:
                raise ValueError(f"Champion portrait is too small: {icon_path}")
    ...
    def _make_output_directories(self):
        ...
        if self.dataset_name:
            ...
            folder_name = f"lol_minimap_{self.output_image_size}_{datetime.datetime.now().strftime('%y%m%d_%H%M%S')}"

        self.save_dir = os.path.join(base_dir, folder_name)
        save_path = Path(self.save_dir)
        if save_path.is_dir() and any(save_path.iterdir()):
            raise FileExistsError(
                f"Synthetic dataset directory is not empty: {save_path}. "
                "Choose another --dataset-name to avoid mixing generated samples."
            )
        if save_path.exists() and not save_path.is_dir():
            raise FileExistsError(f"Synthetic dataset path is not a directory: {save_path}")
    ...
    def _write_dataset_yaml(self):
        ...
        with open(yaml_path, "w") as f:
            dataset_path = str(Path(self.save_dir).resolve()).replace(os.sep, "/")
            f.write(f"path: {json.dumps(dataset_path)}\n")
            f.write("train: train/images\n")
            f.write("val: val/images\n")
            f.write("test: test/images\n")
            f.write(f"nc: {self.num_champions}\n")
            f.write(f"names: {json.dumps(self.champion_names)}\n")

    def _write_generation_manifest(self, allow_icon_overlap: bool):
        manifest = {
            "schemaVersion": 1,
            "generator": {
                "repository": "https://github.com/bsowlx/DeepestLeague",
                "revision": "8cb084f6ae9a89362d30dc2200e775d91cf66f64",
                "adaptation": "Recall local-roster deterministic generator",
            },
            "seed": self.seed,
            "splits": {"train": self.n_train, "val": self.n_val, "test": self.n_test},
            "imageSize": self.output_image_size,
            "classCount": self.num_champions,
            "classNames": self.champion_names,
            "focusChampions": self.focus_champions,
            "focusPerImage": min(self.focus_per_image, len(self.focus_champions)),
            "augmentations": {
                "hsv": self.use_hsv_augmentation,
                "jpegNoise": self.use_noise,
                "background": self.use_background_augment,
                "icon": self.use_icon_augment,
                "iconOverlap": bool(allow_icon_overlap),
                "recallTeleport": self.use_recall_tp,
                "viewportSimulation": self.viewport_sim,
            },
        }
        manifest_path = Path(self.save_dir) / "generation-manifest.json"
        manifest_path.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")

    def _sample_champion_indices(self, split_name: str, sample_index: int) -> np.ndarray:
        """Select ten unique classes and deterministically cover focus classes."""
        focus_count = min(self.focus_per_image, len(self.focus_indices), 10)
        selected_focus: list[int] = []
        if focus_count:
            split_offset = self.split_names.index(split_name)
            anchor = (sample_index + split_offset) % len(self.focus_indices)
            selected_focus = [
                self.focus_indices[(anchor + offset) % len(self.focus_indices)]
                for offset in range(focus_count)
            ]
        remaining = np.asarray(
            [index for index in range(self.num_champions) if index not in selected_focus],
            dtype=np.int64,
        )
        random_count = 10 - len(selected_focus)
        selected_random = np.random.permutation(remaining)[:random_count].tolist()
        selected = np.asarray(selected_focus + selected_random, dtype=np.int64)
        return np.random.permutation(selected)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default=".minimap-training/data/synthetic")
    parser.add_argument("--dataset-name", default=None)
    parser.add_argument(
        "--focus-champions",
        default=None,
        help="Comma-separated champions to oversample; defaults to roster additions",
    )
    parser.add_argument(
        "--focus-per-image",
        type=int,
        default=1,
        help="Number of focus champions guaranteed in every image",
    )
    ...
    focus_champions = None
    if args.focus_champions is not None:
        focus_champions = [
            entry.strip() for entry in args.focus_champions.split(",") if entry.strip()
        ]
    generator = SyntheticDataGenerator(
        focus_champions=focus_champions,
        focus_per_image=args.focus_per_image,
        seed=args.seed,
    )
