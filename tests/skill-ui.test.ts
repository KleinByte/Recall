...
describe("Skill Overview", () => {
  it("leads with an accessible RVI profile and preserves both evidence inspectors", () => {
    ...
    expect(profile).toContain("Your RVI")
    expect(profile).toContain("identity.description")
    expect(profile).toContain('class="profile-lead"')
    expect(profile).toMatch(/PerformanceRadar[\s\S]*class="profile-story"/)
    expect(profile).toContain('class="profile-story" aria-label="RVI profile highlights"')
    expect(profile).toContain('class="story-card identity"')
    expect(profile).toContain('aria-labelledby="rvi-style-title"')
    expect(profile).toContain('<h3 id="rvi-style-title">{{ identity.label }}</h3>')
    expect(profile).toContain('class="story-card strongest"')
    expect(profile).toContain('aria-labelledby="rvi-strongest-title"')
    expect(profile).toContain(':aria-label="`Arm score ${strongest.score}`"')
    expect(profile).toContain('class="story-card movement"')
    expect(profile).toContain('aria-labelledby="rvi-movement-title"')
    expect(profile).toContain("Profile holding steady")
    expect(profile).toContain("profile.comparison")
    expect(profile).toContain("strongest.label")
    expect(profile).toContain("growth.label")
    expect(profile).toContain("Compare RVI")
    expect(profile).toContain('<h3 id="rvi-scopes-title">Compare RVI</h3>')
    expect(profile).toContain('aria-describedby="rvi-scopes-description"')
    expect(profile).toContain(':aria-labelledby="`rvi-scope-${group.key}`"')
    expect(profile).toContain('<ul class="scope-items" role="list">')
    expect(profile).toContain(':aria-label="`RVI score ${scope.score}`"')
    expect(profile).toContain('class="scope-meter" aria-hidden="true"')
    expect(profile).toContain("hasScopeComparisons")
    expect(profile).toContain("scopeComparisonDescription")
    expect(profile).toContain("gameCountLabel")
    expect(profile).toContain(" of ${scope.headline.totalArms} arms")
    expect(profile).toContain("flex: var(--scope-count) 1 170px")
    expect(profile).toContain("min-height: 0")
    expect(profile).toContain("align-items: flex-start")
    expect(profile).toContain("border-radius: 999px")
    expect(profile).toContain(".scope-overall > h4")
    expect(profile).toContain("'--scope-count': group.items.length")
    const scopeRail = profile.slice(
      profile.indexOf('v-if="hasScopeComparisons"'),
      profile.indexOf('<div class="profile-lead">'),
    )
    expect(scopeRail).not.toContain("<button")
    expect(scopeRail).not.toContain("@click")
    expect(scopeRail).not.toContain("tabindex")
    expect(profile).toContain('class="dimension-grid career-dimension-grid"')
    expect(profile).toContain('aria-label="RVI performance arms"')
    expect(profile).toContain('class="dimension-card"')
    expect(profile).toContain('class="dimension-mark" aria-hidden="true"')
    expect(profile).toContain('class="dimension-result"')
    expect(profile).toContain('class="dimension-delta"')
    ...
    expect(profile).toContain("careerDetailsOpen ? 'Hide measurements' : 'Show measurements'")
