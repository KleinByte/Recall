...
describe("Skill Overview", () => {
  it("makes RVI the single performance identity below ranked history", () => {
    ...
    expect(profile).toContain("threat, teamfighting, positioning and survival")
    expect(profile).toContain("pulled toward 50")
    expect(profile).toContain("missing scored evidence withholds")
    expect(profile).toContain("Recorded RVI scopes")
    expect(profile).toContain("Primary archetype")
    expect(profile).toContain("Champion + position")
    expect(profile).toContain("scope.measuredGames")
    expect(profile).toContain("scope.coverage")
    ...
    expect(overview).toContain(':champions="champions"')
