...
} from "../electron/main/matches/grade-v3-calibration.js"
import { gradeForRoleFitScore } from "../electron/main/matches/grade-v3-recipe.js"

describe("Grade v3 match-cluster calibration", () => {
  ...
  it("keeps broad-cohort tails and the median reachable after hierarchical shrinkage", () => {
    ...
    expect(gradeForRoleFitScore(percentile(-1) * 100)).toBe("D")
    expect(gradeForRoleFitScore(percentile(49.5) * 100)).toBe("B+")
    expect(gradeForRoleFitScore(percentile(100) * 100)).toBe("S+")
  })
