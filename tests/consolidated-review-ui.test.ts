...
describe("consolidated match review UI", () => {
  ...
  it("uses one tabbed match destination with the requested review surfaces", () => {
    ...
    expect(review).toContain('type MatchTab = "overview" | "jungle" | "stats" | "timeline" | "probability"')
    expect(review).toContain('label: "Jungle clear"')
  ...
  it("plays positioned timeline frames on a synchronized review map", () => {
    ...
    expect(playback).toContain("unifiedPlaybackTrails")
    expect(playback).toContain("campTimelineTicks")
    ...
    expect(playback).toContain("Riot snapshots anchor one continuous route")
