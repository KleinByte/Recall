...
describe("consolidated match review UI", () => {
  ...
  it("plays positioned timeline frames on a synchronized review map", () => {
    ...
    expect(playback).toContain("expandedParticipantId")
    expect(playback).toContain('@pointerenter="expandTokenCluster')
    expect(playback).toContain("expandedTokenLinks")
    expect(playback).not.toContain("transition: left")
