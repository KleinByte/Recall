...
describe("consolidated match review UI", () => {
  ...
  it("uses one tabbed match destination with the requested review surfaces", () => {
    ...
    expect(review).not.toContain('id: "jungle"')
    expect(review).toContain("<JungleClearTimelineSummary")
    expect(review).toContain('@seek="selectTimelineTimestamp"')
    expect(review).toContain("ownerWasJungling")
  ...
  it("plays positioned timeline frames on a synchronized review map", () => {
    ...
    const playbackClock = read("src/helpers/use-playback-clock.ts")
    ...
    expect(review).toContain("v-if=\"timelineSummary && timelineMapView === 'deaths'\"")
    ...
    expect(review).toContain(':frames="timelineSummary?.frames ?? []"')
    expect(review).toContain(':life-intervals="timelineSummary?.participantLifeIntervals"')
    expect(review).toContain(':minimap-review="minimapReview"')
    expect(review).toContain("api.getJunglePathingReview(gameId)")
    expect(review).toContain('events.on("minimap:pathing-updated"')
    expect(review).toContain("timelineJungleMarkers")
    expect(review).toContain("playbackMaximumTimestamp: sharedTimelineMaximumTimestamp")
    expect(review).toContain('v-model:timestamp="timelineCursorTimestamp"')
    expect(playback).toContain("unifiedPlaybackPositionsAt")
    expect(playback).toContain("unifiedPlaybackTrailSegments")
    expect(playback).toContain("data-segment-key")
    expect(playback).not.toContain("campTimelineTicks")
    expect(playback).toContain("cv_observed")
    expect(playback).toContain("playbackWorldMarkers")
    expect(playback).toContain("usePlaybackClock")
    expect(playback).toContain("renderFps: 30")
    expect(playback).toContain("publishFps: 10")
    expect(playbackClock).toContain("requestAnimationFrame")
    expect(playbackClock).toContain('visibilityState !== "hidden"')
    expect(playback).toContain("[1, 2, 4, 10]")
    expect(playback).toContain("About playback evidence")
    expect(playback).not.toContain("trail-hint")
    expect(playback).not.toContain("evidence-legend")
    expect(playback).not.toContain("visibility-controls")
    expect(playback).toContain('v-if="expandedMap" class="scrubber-shell"')
