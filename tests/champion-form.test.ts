...
describe("Champion Form v2", () => {
  it("uses the inclusive 90-calendar-day boundary", () => {
    const base = buildChampionFormLegacy([], { asOfMs, timeZone: "America/Chicago", family: "sr" })
    const result = buildChampionFormLegacy([
  ...
  it("matches every posterior field exactly", () => {
    ...
    const [row] = buildChampionFormLegacy(games, {
      asOfMs, timeZone: "America/Chicago", family: "sr",
    }).earlySignals
  ...
  it("uses old same-family grades only for the prior", () => {
    const preview = buildChampionFormLegacy([], { asOfMs, timeZone: "UTC", family: "sr" })
    const result = buildChampionFormLegacy([
      game({ gradeScore: 2 }),
  ...
  it("classifies 1–4 as early, 5+ as main, and confidence at 5/12", () => {
    ...
    const result = buildChampionFormLegacy(games, { asOfMs, timeZone: "UTC", family: "sr" })
  ...
  it("returns an explicit empty state for all-Arena history", () => {
    expect(buildChampionFormLegacy([
