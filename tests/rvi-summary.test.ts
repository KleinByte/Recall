...
} from "../electron/main/matches/rvi-summary.js"
import { metricDefinition } from "../electron/main/matches/match-metric-registry.js"

const participant = (
  participantId: number,
  overrides: Partial<SummaryMetricParticipant> = {},
): SummaryMetricParticipant => ({
  participantId,
...
describe("Recall summary metric derivation", () => {
  it("emits the full inventory, duration-normalized formulas, and observed zero", () => {
    const result = deriveSummaryMetricObservations({
      participantId: 1,
      durationSecs: 1_200,
    ...
    expect(result).toHaveLength(SUMMARY_METRIC_KEYS.length)
    ...
    for (const observation of result) {
      expect(observation.source, observation.metricKey)
        .toBe(metricDefinition(observation.metricKey)?.source)
    }
  })

  it("marks Rift-only metrics not applicable in Howling Abyss", () => {
    const result = new Map(deriveSummaryMetricObservations({
      participantId: 1,
      durationSecs: 1_200,
