...
import {
  RVI_FIGHT_CLUSTER_RADIUS,
  RVI_FIGHT_CLUSTER_WINDOW_MS,
  TIMELINE_METRIC_KEYS,
  clusterTimelineFights,
  deriveTimelineMetricObservations,
} from "../electron/main/matches/rvi-timeline.js"
...
function byKey(result: ReturnType<typeof deriveTimelineMetricObservations>) {
  return new Map(result.map((entry) => [entry.metricKey, entry]))
}

const rift = defaultGradeModeContext("sr")

describe("Recall corrected timeline diagnostics", () => {
  it("clusters deterministically at the exact temporal and spatial boundaries", () => {
    ...
    const boundary = event("b", RVI_FIGHT_CLUSTER_WINDOW_MS, "CHAMPION_KILL", {
      participantId: 2,
      targetId: 7,
      assistingParticipantIds: [1],
      teamId: 100,
      position: { x: RVI_FIGHT_CLUSTER_RADIUS, y: 0 },
    })
    const clustered = clusterTimelineFights([first, boundary])
    const permuted = clusterTimelineFights([boundary, first])
    expect(clustered).toEqual(permuted)
    expect(clustered.state === "observed" && clustered.value).toHaveLength(1)
    expect(clusterTimelineFights([first, {
      ...boundary,
      timestamp: RVI_FIGHT_CLUSTER_WINDOW_MS + 1,
    }])).toMatchObject({ state: "observed", value: [{}, {}] })
    expect(clusterTimelineFights([first, {
      ...boundary,
      position: { x: RVI_FIGHT_CLUSTER_RADIUS + 1, y: 0 },
    }])).toMatchObject({ state: "observed", value: [{}, {}] })
    expect(clusterTimelineFights([{ ...first, position: undefined }]))
      .toMatchObject({
        state: "unavailable",
        reason: "incomplete_spatial_fight_evidence:0_of_1",
      })
    expect(clusterTimelineFights([first, { ...boundary, position: undefined }]))
      .toMatchObject({
        state: "unavailable",
  ...
  it("withholds fight scores when spatial event coverage is incomplete", () => {
    ...
    const metrics = byKey(deriveTimelineMetricObservations({
      participantId: 1,
      teamId: 100,
      durationSecs: 20 * 60,
      context: rift,
  ...
  it("counts the player as an ally when identifying outnumbered deaths", () => {
    ...
    const derive = (assists: number[]) => byKey(deriveTimelineMetricObservations({
      participantId: 1,
      teamId: 100,
      durationSecs: 20 * 60,
      context: rift,
  ...
  it("returns the full inventory and derives phase, objective, setup, and early evidence", () => {
    ...
    const result = deriveTimelineMetricObservations({
      participantId: 1,
      teamId: 100,
      durationSecs: 32 * 60,
      context: rift,
    ...
    expect(result).toHaveLength(TIMELINE_METRIC_KEYS.length)
  ...
  it("keeps mode exclusions, missing opponent evidence, and zero opportunities distinct", () => {
    ...
    const aramMetrics = byKey(deriveTimelineMetricObservations({
      participantId: 1,
      teamId: 100,
      durationSecs: 20 * 60,
    ...
    const riftMetrics = byKey(deriveTimelineMetricObservations({
      participantId: 1,
      teamId: 100,
      durationSecs: 20 * 60,
      context: rift,
  ...
  it("returns a reasoned full inventory when no timeline was retained", () => {
    ...
    expect(riftMetrics.size).toBe(TIMELINE_METRIC_KEYS.length)
  ...
  it("counts the exact pre-objective temporal and spatial boundary", () => {
    ...
    const atBoundary = byKey(deriveTimelineMetricObservations({
      participantId: 1,
      teamId: 100,
      durationSecs: 20 * 60,
      context: rift,
    ...
    expect(atBoundary.get("pre_objective_deaths_per_opportunity")?.rawEvidence)
      .toMatchObject({ state: "observed", value: 1 })
    const outside = byKey(deriveTimelineMetricObservations({
      participantId: 1,
      teamId: 100,
      durationSecs: 20 * 60,
      context: rift,
