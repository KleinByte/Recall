...
describe("minimap pathing review fallback", () => {
  ...
  it("renders observed and estimated routes while keeping rejected gaps separate", async () => {
    ...
    expect(wrapper.findAll(".path-layer path")).toHaveLength(2)
    expect(wrapper.findAll(".path-layer path.estimated")).toHaveLength(1)
    ...
    expect(wrapper.text()).toContain("Rejected gaps stay disconnected")
    expect(wrapper.find("canvas").exists()).toBe(false)
  })
