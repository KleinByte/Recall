...
import { recommendationDirectionV3 } from "../electron/main/review/recommendations.js"
...
describe("recommendation direction v3", () => {
  it("uses fixed latest and preceding slices and requires interval evidence", () => {
    ...
    expect(recommendationDirectionV3(games, "sr", 1, 0)).toMatchObject({
      direction: "up", latestGames: 10, precedingGames: 10, draws: 2_000,
    })
    expect(recommendationDirectionV3(games.slice(0, 14), "sr", 1, 0).direction)
      .toBe("unknown")
  })
})
