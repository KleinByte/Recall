...
import {
  rawResponsibilityScores,
  scoreMatchLobby,
} from "../electron/main/matches/match-grade.js"
import { shrunkMidEcdf } from "../electron/main/matches/match-grade-calibration.js"
...
import { calibrationScopeKey } from "../electron/main/matches/match-grade-taxonomy.js"
...
function lobby(
  matchId: number,
  participantOverrides: Partial<GradeRawParticipant> = {},
  contextOverrides: Partial<GradeRawLobby["context"]> = {},
): GradeRawLobby {
  return {
...
describe("match Grade raw observations", () => {
  it("keeps explicit zero as observed evidence", () => {
    const evidence = deriveRawMetricEvidence(lobby(1, {
      deaths: 0,
      damageToChampions: 0,
      damageObjectives: 0,
      damageTurrets: 0,
  ...
  it("separates neutral-objective damage from structure damage and clamps overlap", () => {
    const split = deriveRawMetricEvidence(lobby(1, {
      damageObjectives: 1_500,
      damageTurrets: 500,
      damageStructures: 800,
    })).get(1)
    const fullyOverlapped = deriveRawMetricEvidence(lobby(2, {
  ...
  it("does not turn one missing ally-sustain field into an observed zero", () => {
    const missingHeal = deriveRawMetricEvidence(lobby(1, {
      totalHealsOnTeammates: undefined,
      totalDamageShieldedOnTeammates: 0,
    })).get(1)?.ally_heal_shield_per_min
    const missingShield = deriveRawMetricEvidence(lobby(2, {
      totalHealsOnTeammates: 0,
      totalDamageShieldedOnTeammates: undefined,
    })).get(1)?.ally_heal_shield_per_min
  ...
  it("does not let missing diagnostic sustain reduce grade evidence coverage", () => {
    const snapshot = buildGradeCalibrationSnapshot(
      Array.from({ length: 10 }, (_, index) => lobby(index + 1)),
    )
    const subject = lobby(100)
    const withoutSustain: GradeRawLobby = {
      ...subject,
    ...
    const prepared = prepareGradeLobbyFromSnapshot(withoutSustain, snapshot)
  ...
  it("uses a neutral percentile only when the team had no opportunity", () => {
    ...
    const prepared = prepareGradeLobbyFromSnapshot(noKills, snapshot)
    expect(prepared.evidenceCoverage).toBe(1)
  ...
  it("calibrates the raw responsibility composite into a true frozen-reference percentile", () => {
    ...
    const prepared = prepareGradeLobbyFromSnapshot(subject, snapshot)
    const raw = rawResponsibilityScores({
      players: prepared.players,
    ...
    expect(result.recallScore).toBeCloseTo(expected.percentile * 100)
    expect(result.grade).toBe(gradeForRecallScore(result.recallScore))
  })
  ...
  it("withholds Recall Score when a snapshot lacks a composite reference", () => {
    const snapshot = buildGradeCalibrationSnapshot(
      Array.from({ length: 10 }, (_, index) => lobby(index + 1)),
    )
    const malformed: typeof snapshot = { ...snapshot, compositeObservations: [] }
    const prepared = prepareGradeLobbyFromSnapshot(lobby(100), malformed)
  ...
  it("never mutates the frozen reference while grading later matches", () => {
    ...
    const first = prepareGradeLobbyFromSnapshot(lobby(100, {
      damageToChampions: 1,
    }), snapshot)
    const second = prepareGradeLobbyFromSnapshot(lobby(101, {
  ...
  it("builds the same content-addressable snapshot regardless of input order", () => {
    ...
    const forward = buildGradeCalibrationSnapshot(references)
    const reversed = buildGradeCalibrationSnapshot([...references].reverse().map((entry) => ({
      ...entry,
  ...
  it("freezes only exact mode and ruleset scopes with ten independent matches", () => {
    ...
    const snapshot = buildGradeCalibrationSnapshot([...ruleOne, ...ruleTwo])
    ...
    const unrepresented = prepareGradeLobbyFromSnapshot(lobby(999, {}, {
      rulesetKey: "sr_normal:rules-r2",
    }), snapshot)
  ...
  it("does not freeze an aggregate of individually underfilled scopes", () => {
    ...
    const snapshot = buildGradeCalibrationSnapshot([...normal, ...aram])
  ...
  it("does not merge equal numeric game ids from different platforms", () => {
    ...
    const snapshot = buildGradeCalibrationSnapshot([...northAmerica, ...europe])

    expect(snapshot.clusterIds).toHaveLength(10)
    expect(snapshot.referencePopulation.scopeMatchCounts).toEqual({
      [calibrationScopeKey(northAmerica[0].context)]: 10,
    })
    const prepared = prepareGradeLobbyFromSnapshot(northAmerica[0], snapshot)
  ...
  it("deduplicates Match-V5 and LCU identities for the same lobby", () => {
    const matchV5 = gradeCalibrationClusterId({
      gameId: 123,
      puuid: "match-v5-owner",
      riotMatchId: "na1_123",
    })
    const lcu = gradeCalibrationClusterId({
      gameId: 123,
      puuid: "lcu-owner",
      platformId: "na1",
    })
    const otherRegion = gradeCalibrationClusterId({
      gameId: 123,
      puuid: "euw-owner",
      platformId: "EUW1",
    })
    ...
    const references = Array.from({ length: 10 }, (_, index) => lobby(index + 1))
    const duplicateLcuView: GradeRawLobby = {
      ...references[0],
      puuid: "second-local-account",
      clusterId: gradeCalibrationClusterId({
        gameId: 1,
        puuid: "second-local-account",
        platformId: "NA1",
      }),
    }
    const snapshot = buildGradeCalibrationSnapshot([...references, duplicateLcuView])
    expect(snapshot.clusterIds).toHaveLength(10)
    expect(snapshot.compositeObservations).toHaveLength(100)
  })
})
