...
import {
  ...
  collectDetailMetricCalibrationRows,
  detailMetricCalibrationCohort,
  toMatchMetricObservation,
  validateMatchMetricObservation,
  type RawMetricObservation,
} from "../electron/main/matches/match-metric-observations.js"
...
describe("Recall metric observations", () => {
  it("preserves an observed raw and calibrated zero", () => {
    const observation = toMatchMetricObservation(
      raw("damage_share", 0),
      {
        gameId: 1,
        puuid: "owner",
    ...
    expect(() => validateMatchMetricObservation(observation)).not.toThrow()
  ...
  it("collects only observed finite rows and retains zero in the frozen reference", () => {
    const rows = collectDetailMetricCalibrationRows([
      {
        matchId: "NA1_1",
  ...
  it("builds archetype to position to mode cohorts and excludes the subject match", () => {
    ...
    const rows = collectDetailMetricCalibrationRows(matches)
    const target = {
      matchId: "NA1_0",
      scopeKey: "ranked:rules-r1",
      position: "MIDDLE",
    ...
    expect(detailMetricCalibrationCohort("damage_share", target, rows)).toMatchObject({
      comparisonScope: "archetype",
      referenceMatchCount: 3,
      rootReferenceMatchCount: 11,
    })
    const higher = calibrateRawDetailMetric(raw("damage_share", .05), target, rows, {
      minimumReferenceMatches: 1,
      kappa: 0,
      direction: "higher",
    })
    const lower = calibrateRawDetailMetric(raw("damage_share", .05), target, rows, {
      minimumReferenceMatches: 1,
      kappa: 0,
      direction: "lower",
    })
  ...
  it("preserves raw evidence when the reference is too small or the source is absent", () => {
    ...
    const result = calibrateRawDetailMetric(raw("damage_share", .2), target, [], {
    ...
    const absent = calibrateRawDetailMetric({
